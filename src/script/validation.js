...
const customValidationBodyHandler = (event) => {
    ...
    if(event.target.validity.tooShort) {
        event.target.setCustomValidity('minimal panjang karakter Isi catatan adalah 20')
        return
    }

    if(event.target.validity.tooLong) {
        event.target.setCustomValidity('maksimal panjang karakter Isi catatan adalah 200')
        return
    }
}
